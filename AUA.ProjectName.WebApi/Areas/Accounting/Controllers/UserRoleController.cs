...
namespace AUA.ProjectName.WebApi.Areas.Accounting.Controllers
{
    ...
    public class UserRoleController : BaseApiController
    {
        ...
        [HttpPost]
        public async Task<ResultModel<bool>> InsertAsync(UserRoleInsertManyVm userRole)
        {

            await _userRoleService
                .InsertCustomVmAsync(userRole);

            return CreateSuccessResult(true);
        }

        [HttpPost]
        public async Task<ResultModel<bool>> DeleteAsync(UserRoleInsertManyVm userRoleVm)
        {

            var userRole = await _userRoleService.GetAll().Where(p => p.AppUserId == userRoleVm.AppUserId)
                .FirstAsync(p => p.RoleId == userRoleVm.RoleId);
